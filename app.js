...
const categoryNotes = {
  ...
  广告片: "用 AIGC 建立品牌叙事、产品想象与情绪记忆。",
  漫剧: "围绕 IP、怪谈与类型叙事，探索 AI 影像的连续剧感。",
};

const categories = ["创意短片", "广告片", "漫剧"];
...
const getBilibiliEmbedUrl = (video) => {
  if (video.platform !== "bilibili" || !video.bvid.trim()) {
    return "";
  }

  const bvid = encodeURIComponent(video.bvid.trim());
  const page = encodeURIComponent(String(video.page || 1));

  return `https://player.bilibili.com/player.html?bvid=${bvid}&p=${page}&poster=1&autoplay=0&danmaku=0`;
};

const renderWorkCard = (work) => {
  const isPending = !work.bvid.trim();
  const statusLabel = isPending ? "视频待更新" : "B站播放";
...
const openVideoModal = (video, trigger) => {
  const embedUrl = getBilibiliEmbedUrl(video);

  lastFocusedElement = trigger;
  modalMeta.textContent = `${video.category} / ${video.tag}`;
  ...
  if (embedUrl) {
    ...
    iframe.src = embedUrl;
    iframe.title = `${video.title} - B站播放器`;
    iframe.loading = "lazy";
    ...
    pendingNotice.innerHTML = "<strong>视频待更新</strong><span>作品视频即将上线</span>";
