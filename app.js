...
const getPlatform = (video) => (video.platform === "douyin" ? "douyin" : "bilibili");
...
const getIframeTitle = (video) => {
  const platform = platformLabels[getPlatform(video)];

  return `${video.title} - ${platform}播放器`;
};
...
const openVideoModal = (video, trigger) => {
  ...
  if (embedUrl) {
    ...
    pendingNotice.innerHTML = "<strong>视频即将上线</strong><span>后续填入平台视频 ID 后即可在这里播放</span>";
